...
export class CapteurService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/capteurs');
  ...
  findN(): Observable<EntityResponseType> {
    return this.http.get<ICapteur>(`${this.resourceUrl}/not`, { observe: 'response' });
  }
  ...
  query2(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<ICapteur[]>(`${this.resourceUrl}/not`, { params: options, observe: 'response' });
  }
